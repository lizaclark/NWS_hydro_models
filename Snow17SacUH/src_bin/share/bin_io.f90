...
subroutine read_sac_params(param_file_name,n_hrus)
  ...
  ! open parameter file
  open(unit=50,file=trim(param_file_name), format='unformatted', &
       status='old')
  ...
  n_params_read = 0
  do while(ios == 0)
    read(unit=50,IOSTAT=ios) readline
...
subroutine read_snow17_params(param_file_name,n_hrus)
  ...
  ! open parameter file
  open(unit=51,file=trim(param_file_name),format='unformatted', &
       status='old')
  ...
  n_params_read = 0
  do while(ios .eq. 0)
    read(unit=51,IOSTAT=ios) readline
...
subroutine read_uh_params(param_file_name,n_hrus)
  ...
  ! open parameter file
  open(unit=52,file=trim(param_file_name), format='unformatted',&
       status='old')
  ...
  n_params_read = 0
  do while(ios .eq. 0)
    read(unit=52,IOSTAT=ios) readline
